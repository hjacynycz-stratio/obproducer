...
## POSTGRES DB =====================================================
    ...
    export POSTGRES_URL="jdbc:postgresql://tenantproducer-poolretail.tenantproducer.marathon.mesos:5432/ontology"
    export SPRING_DATASOURCE_USERNAME="${MARATHON_SERVICE_NAME}_tenantproducer"

    export POSTGRES_CERT="/etc/stratio/${MARATHON_SERVICE_NAME}_tenantproducer.pem"
    ...
    getCert "userland" \
                "${MARATHON_SERVICE_NAME}.tenantproducer" \
                "${MARATHON_SERVICE_NAME}_tenantproducer" \
                "PEM" \
                "/etc/stratio" \
    && INFO "    - OK" \
    || INFO "    - Error"
    ...
    openssl pkcs8 -topk8 -inform pem -in /etc/stratio/"${MARATHON_SERVICE_NAME}_tenantproducer".key -outform der -nocrypt -out ${POSTGRES_KEY}
...
## KAFKA SSL COLLECTIONS ===========================================
    ...
    CERTIFICATE_VAR_PASS=${MARATHON_SERVICE_NAME//-/_}"_tenantproducer_KEYSTORE_PASS"
    ...
    export SPRING_KAFKA_BOOTSTRAP_SERVERS=kafka-0-broker.tenantproducer-kafka.mesos:9092,kafka-1-broker.tenantproducer-kafka.mesos:9092,kafka-2-broker.tenantproducer-kafka.mesos:9092

    openssl pkcs12 -export -out /etc/stratio/kafka.pkcs12 \
    -in /etc/stratio/"${MARATHON_SERVICE_NAME}_tenantproducer".pem \
    -inkey /etc/stratio/"${MARATHON_SERVICE_NAME}_tenantproducer".key \
